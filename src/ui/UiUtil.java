package ui;


import models.AssosiacoesModel;
import service.AssociacaoService;

public class UiUtil {

    public void home() {
        System.out.println("");
        System.out.println("");
        System.out.println("******************************************************************");
        System.out.println("*                                                                *");
        System.out.println("*            Bem Vindo ao Gestão de Doação                       *");
        System.out.println("*                                                 By:TechChoi =]  *");
        System.out.println("******************************************************************");
        System.out.println("");
        System.out.println("");

    }

    public void menuInicial(){
        System.out.println("");
        System.out.println("");
        System.out.println("******************************************************************");
        System.out.println("*                                                                *");
        System.out.println("*                       Menu inicial                             *");
        System.out.println("*                                                                *");
        System.out.println("******************************************************************");
        System.out.println("");
        System.out.println("");

        System.out.println("");
        System.out.println("");
        System.out.println("\nDigite uma das opções abaixo: ");
        System.out.println("\n1 - Acessar menu de Associações");
        System.out.println("\n2 - Acessar menu de parceiros");
        System.out.println("\n3 - Acessar menu de funcionários");
        System.out.println("\n4 - Acessar menu de Doação");
        System.out.println("");
        System.out.println("");

    }
    public void menuAssociacao(){
        System.out.println("");
        System.out.println("");
        System.out.println("******************************************************************");
        System.out.println("*                                                                *");
        System.out.println("*                       Menu Associação                           *");
        System.out.println("*                                                                *");
        System.out.println("******************************************************************");
        System.out.println("");
        System.out.println("");

        System.out.println("");
        System.out.println("");
        System.out.println("\nDigite uma das opções abaixo: ");
        System.out.println("\n1 - Lista de Associações");
        System.out.println("\n2 - Cadastrar Associação");
        System.out.println("\n3 - Deletar Associação");
        System.out.println("\n4 - Voltar ao menu principal");
        System.out.println("");
        System.out.println("");

    }
    public void menuParceiros(){
        System.out.println("");
        System.out.println("");
        System.out.println("******************************************************************");
        System.out.println("*                                                                *");
        System.out.println("*                       Menu Parceiros                             *");
        System.out.println("*                                                                *");
        System.out.println("******************************************************************");
        System.out.println("");
        System.out.println("");

        System.out.println("");
        System.out.println("");
        System.out.println("\nDigite uma das opções abaixo: ");
        System.out.println("\n1 - Lista de parceiros");
        System.out.println("\n2 - Cadastrar parceiros");
        System.out.println("\n3 - Deletar parceiros");
        System.out.println("\n4 - Voltar ao menu principal");
        System.out.println("");
        System.out.println("");

    }
    public void menuFuncionarios(){
        System.out.println("");
        System.out.println("");
        System.out.println("******************************************************************");
        System.out.println("*                                                                *");
        System.out.println("*                       Menu Funcionários                         *");
        System.out.println("*                                                                *");
        System.out.println("******************************************************************");
        System.out.println("");
        System.out.println("");

        System.out.println("");
        System.out.println("");
        System.out.println("\nDigite uma das opções abaixo: ");
        System.out.println("\n1 - Lista de funcionários");
        System.out.println("\n2 - Cadastrar funcionários");
        System.out.println("\n3 - Deletar funcionários");
        System.out.println("\n4 - Voltar ao menu principal");
        System.out.println("");
        System.out.println("");

    }
    public void menuDoacoes(){
        System.out.println("");
        System.out.println("");
        System.out.println("******************************************************************");
        System.out.println("*                                                                *");
        System.out.println("*                       Menu Doações                             *");
        System.out.println("*                                                                *");
        System.out.println("******************************************************************");
        System.out.println("");
        System.out.println("");

        System.out.println("");
        System.out.println("");
        System.out.println("\nDigite uma das opções abaixo: ");
        System.out.println("\n1 - Lista de doações");
        System.out.println("\n2 - Cadastrar doações");
        System.out.println("\n3 - Dados bancários para doações");
        System.out.println("\n4 - Voltar ao menu principal");
        System.out.println("");
        System.out.println("");

    }

    public void cadastrarAssociacao(){
        System.out.println("Menu de cadastro de Associação");
    }
    public void listarAssociacao(){
        System.out.println("Menu de listagem de Associações");
    }


    public void clear(){
        for(int i =0; i<=100; i++){
            System.out.println("");
        }
    }
}
